...
def exception_handle(self_, err_msg=None, exit_code=None,
    ...
    def process_exception(e, self=None):
        ...
        if conn_err:
            ...
            #f=open('ip.txt','a')    # r只读，w可写，a追加
            #f.write(addr+'\n')
            #f.close()
            logging.error('%s when shell handling connection from %s:%d' %
                          (e, addr, port))
...
def get_config(is_local):
    ...
    if is_local and not config:
        logging.error('config not specified')
        print_help(is_local)
        sys.exit(2)

    config['password'] = to_bytes(config.get('password', b'kxsw.me'))
    ...
    config['server_port'] = config.get('server_port', 443)
